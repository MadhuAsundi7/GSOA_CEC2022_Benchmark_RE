...
if(populLimCount[curIndx]>MIN_ITS_ON_BOUND){
    ...
    return;
        ...
        for( int cand_k=2; cand_k<=MAX_K; ++cand_k){
            ...
            double silhouetteScore = mlpack::SilhouetteScore::Overall(data, assignments, metric);
...

    {
        ...
        int oldPopSize=POP_SIZE;
            ...
            for(int func_num=1;func_num<13;func_num++)
            {
                ...
                sprintf(buffer,"%d_%d_pop_%d.txt",func_num,dimensionality, POP_SIZE);
